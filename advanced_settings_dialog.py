...
class AdvancedSettingsDialog(QDialog):
    ...
    def update_cookie_option_info(self, mode: str) -> None:
        ...
        self.advanced_cookie_info_body.setText(descriptions.get(mode, descriptions["none"]))
    ...
    def update_cookie_file_status(self) -> None:
        ...
        if mode == "none":
            message = "当前不会向下载流程传递 Cookies。推荐用于 YouTube / 普通公开视频。"
        elif mode == "browser":
            browser = self.advanced_cookie_browser_combo.currentData() or "浏览器"
            browser_name = str(browser).title()
            message = (
                f"将尝试从 {browser_name} 读取登录状态；只有读取成功后本次请求才会使用 Cookies。"
                "Chrome / Edge 可能因数据库占用或 DPAPI 解密失败而无法读取。"
            )
        elif not path:
            message = "请选择 cookies.txt 文件；推荐用于 Bilibili、登录资源、受限视频。软件只保存路径，不读取或展示 Cookies 内容。"
