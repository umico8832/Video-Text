...
from advanced_cookies_tab import (
    COOKIE_BROWSERS,
    build_cookie_access_tab as create_cookie_access_tab,
)
...
COOKIE_MODES = [
    ...
    {"name": "browser", "label": "从浏览器读取（可能失败）"},
]
...
class AdvancedSettingsDialog(QDialog):
    ...
    def build_cookie_access_tab(self) -> QWidget:
        return create_cookie_access_tab(self)
    ...
    def update_cookie_access_controls(self) -> None:
        ...
        self.update_cookie_option_info(mode)
        self.update_cookie_file_status()

    def update_cookie_option_info(self, mode: str) -> None:
        if not hasattr(self, "advanced_cookie_info_body"):
            return
        self.advanced_cookie_info_body.setText(cookie_option_description(mode))
    ...
    def update_cookie_option_styles(self, selected_mode: str) -> None:
        ...
        for mode, frame in self.advanced_cookie_option_frames.items():
            ...
            frame.setStyleSheet(cookie_option_frame_style(selected))
            radio = self.advanced_cookie_radios.get(mode)
            if radio is not None:
                radio.setStyleSheet(cookie_option_radio_style(selected))
    ...
    def update_cookie_file_status(self) -> None:
        ...
        browser = self.advanced_cookie_browser_combo.currentData() or "浏览器"
        self.advanced_cookies_status.setText(cookie_file_status(mode, path, str(browser)))
