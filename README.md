...
这是一个 Windows 桌面工具，用于从 B 站、YouTube 等 `yt-dlp` 支持的视频页面提取字幕纯文本，默认面向中文字幕。

软件会优先下载视频已有的目标语言字幕；没有合适字幕时，再下载音频并使用 `faster-whisper` 进行语音识别。普通模型默认处理中文，`.en` 英文专用模型默认处理英文。最终结果保存为 `.txt` 文件。

## 软件是干什么的

- 输入视频链接，提取字幕纯文本。
- 优先使用视频已有的人工字幕或自动字幕，速度更快。
- 没有可用目标语言字幕时，可使用 Whisper 模型识别音频。
...
“Whisper 模型”只在视频没有可直接下载的目标语言字幕时使用。普通模型默认查找中文字幕；`.en` 英文专用模型默认查找英文字幕。
...
- `turbo`：在速度和识别质量之间取平衡。
- 名称带 `.en` 的模型：英文专用，选择后会优先查找英文字幕，找不到时使用英文识别；不建议用于中文视频。
...
1. 使用 yt-dlp 获取视频信息。
2. 按所选模型查找目标语言字幕：普通模型查找中文字幕，`.en` 英文专用模型查找英文字幕。
3. 优先选择人工字幕。
4. 没有合适的人工字幕时，选择视频平台提供的自动字幕。
5. 没有可直接下载的目标语言字幕时，才下载音频并使用 Whisper；普通模型使用中文识别，`.en` 英文专用模型使用英文识别。

选择了 Whisper 模型不会跳过已有字幕；`.en` 英文专用模型只会把目标语言从中文切换为英文。
...
可以。软件会先尝试下载已有目标语言字幕。只有没有可用字幕、必须使用 Whisper 时，才需要模型。

### 提示“未找到可用字幕，需要选择识别模型”

视频没有可直接下载的目标语言字幕，并且当前没有选择 Whisper 模型。选择一个官方模型或本地模型目录后重试。
